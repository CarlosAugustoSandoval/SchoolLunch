<template>
  <q-dialog v-model="model" persistent>
    <base-card
      ...
      title="Registar Tarjeta"
      subtitle="Registar Tarjeta"
      avatar
...
import Card from 'src/modules/cards/models/Card'
...
const item = ref(Card.create(null))
  ...
  if (value) {
    ...
    request
      .then(() => {
        Notify.create({
          type: 'positive',
          message: `La tarjeta se ${dataCopy.id ? 'actualizó' : 'creó'} correctamente.`
        })
        model.value = false
        // emit('saved')
...
const getItem = () => {
  if (props.item?.id) {
    ...
    api.get(`roles/${props.item.id}`)
      .then(({ data }) => {
        if (data?.data) item.value = Card.create(data?.data?.rol)
      })
      .catch(error => Notify.create(notifyError(error)))
      .finally(() => { loading.value = false })
  }
